...
import com.android.aws.interceptors.HttpLoggingInterceptor;
...
import static com.android.aws.common.Method.DELETE;
import static com.android.aws.common.Method.GET;
import static com.android.aws.common.Method.HEAD;
import static com.android.aws.common.Method.OPTIONS;
import static com.android.aws.common.Method.PATCH;
import static com.android.aws.common.Method.POST;
import static com.android.aws.common.Method.PUT;
...
public final class InternalNetworking {
    ...
    public static Response performSimpleRequest(AWSRequest request) throws AWSError {
        ...
        try {
            ...
            switch (request.getMethod()) {
                ...
                case OPTIONS: {
                    builder = builder.method(AWSConstants.OPTIONS, null);
                    break;
                }
                case PATCH: {
                    requestBody = request.getRequestBody();
            ...
            throw new AWSError(ioe);
        }
        return okHttpResponse;
    }

    public static Response performDownloadRequest(final AWSRequest request) throws AWSError {
        Request okHttpRequest;
        Response okHttpResponse;
        try {
            Request.Builder builder = new Request.Builder().url(request.getUrl());
    ...
    public static Response performUploadRequest(AWSRequest request) throws AWSError {
        Request okHttpRequest;
        Response okHttpResponse;
        try {
            Request.Builder builder = new Request.Builder().url(request.getUrl());
    ...
    public static void addHeadersToRequestBuilder(Request.Builder builder, AWSRequest request) {
        if (request.getUserAgent() != null) {
            builder.addHeader(AWSConstants.USER_AGENT, request.getUserAgent());
        } else if (sUserAgent != null) {
            request.setUserAgent(sUserAgent);
            builder.addHeader(AWSConstants.USER_AGENT, sUserAgent);
        }
        Headers requestHeaders = request.getHeaders();
        if (requestHeaders != null) {
            builder.headers(requestHeaders);
            if (request.getUserAgent() != null && !requestHeaders.names().contains(AWSConstants.USER_AGENT)) {
                builder.addHeader(AWSConstants.USER_AGENT, request.getUserAgent());
            }
        }
    }
    ...
    public static void setClientWithCache(Context context) {
        sHttpClient = new OkHttpClient().newBuilder()
                .cache(Utils.getCache(context, AWSConstants.MAX_CACHE_SIZE, AWSConstants.CACHE_DIR_NAME))
                .connectTimeout(60, TimeUnit.SECONDS)
                .readTimeout(60, TimeUnit.SECONDS)
                .writeTimeout(60, TimeUnit.SECONDS)
                .build();
