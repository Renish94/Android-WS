...
public class Utils {
    ...
    public static AWSResponse<Bitmap> decodeBitmap(Response response, int maxWidth,
    ...
    public static AWSResponse<Bitmap> decodeBitmap(Response response, int maxWidth,
        ...
        if (bitmap == null) {
            return AWSResponse.failed(Utils.getErrorForParse(new AWSError(response)));
        } else {
            return AWSResponse.success(bitmap);
        }
    }
    ...
    public static AWSError getErrorForConnection(AWSError error) {
        error.setErrorDetail(AWSConstants.CONNECTION_ERROR);
        error.setErrorCode(0);
        return error;
    }


    public static AWSError getErrorForServerResponse(AWSError error, AWSRequest request, int code) {
        error = request.parseNetworkError(error);
        error.setErrorCode(code);
        error.setErrorDetail(AWSConstants.RESPONSE_FROM_SERVER_ERROR);
        return error;
    }

    public static AWSError getErrorForParse(AWSError error) {
        error.setErrorCode(0);
        error.setErrorDetail(AWSConstants.PARSE_ERROR);
        return error;
    }
